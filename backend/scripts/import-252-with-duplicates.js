...
// EXACT 252 STUDENTS FROM USER DATA - SAME ROLL NO IN DIFFERENT SECTIONS ALLOWED
const students = [
  // C-1: 33 students
...
async function main() {
  console.log('📚 IMPORTING 252 UNIQUE STUDENTS (SAME ROLL NO IN DIFFERENT SECTIONS OK)\n');
  ...
  try {
    ...
    if (!class1st) throw new Error('1st Year class not found');
    ...
    for (const sectionName of sections) {
      let section = await prisma.section.findFirst({
        where: { name: sectionName, classId: class1st.id }
      });
      if (!section) {
        section = await prisma.section.create({
          data: { name: sectionName, classId: class1st.id }
        });
      }
